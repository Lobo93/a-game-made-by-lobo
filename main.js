...
function addBeer(event) {
	...
	let clickedBeer = level.beers.find(beer => beer.x === x && beer.y === y)
	if (clickedBeer) {
		level.beers.splice(level.beers.indexOf(clickedBeer), 1)
		return
	}
	level.beers.push({x:x, y:y})
}
...
const sprites = {
	...
	beer0: {x:32, y:96, width:16, height:16},
	beer1: {x:48, y:96, width:16, height:16},
	beer2: {x:64, y:96, width:16, height:16},
...
const animations = {
	...
	beer: {
		name: 'beer',
		frames: ['beer0', 'beer1', 'beer0', 'beer2'],
		time: 0,
		frame: 0,
		speed: 6,
		directional: false
...
const sharedAnimations = [animations.beer, animations.water, animations.lava, animations.flag]
...
function titleScreenFrames(currentTime) {
	...
	context.strokeText('A Game', 139, 55)
	context.fillText('A Game', 140, 54)

	drawSprite('playerJumpRight', 40, 46, 2)

	context.font = '16px potato-pixel'
	context.strokeText('Made by Lobo', 151, 79)
	context.fillText('Made by Lobo', 152, 78)
...
async function loadLevel(fileName) {
	...
	if (levelJSON.beers) totalBeers += levelJSON.beers.length
...
function gameFrames(currentTime) {
	...
	// Pick beers
	level.beers?.forEach((beer, index) => {
		if (player.dead) return
		if (Math.abs(beer.x - player.x) > 10 || Math.abs(beer.y - player.y) > 10) return
		level.beers.splice(index, 1)
		player.beers += 1
		playSound('glug')
	})
	...
	// Draw beers
	level.beers?.forEach(beer => {
		drawSprite(animations.beer.frames[animations.beer.frame], beer.x - 8 - camera.left, beer.y - 16)
	})
	...
	if (levelJSON !== gameOverLevel) {
		drawSprite('beer0', 0, 0)
		context.strokeText(player.beers + collectedBeers, 18, 7)
		context.fillText(player.beers + collectedBeers, 18, 7)
		...
		let displayBeers = `${collectedBeers}/${totalBeers}`
		let textPosition = 136 - Math.max(context.measureText(displayTime).width, context.measureText(displayBeers).width, context.measureText(deaths).width) / 2
		let iconPosition = textPosition - 18
		drawSprite('beer0', iconPosition, 24)
		context.strokeText(displayBeers, textPosition, 32)
		context.fillText(displayBeers, textPosition, 32)
